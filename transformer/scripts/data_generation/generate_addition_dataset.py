...
def create_monomial(variables, max_degree, coefficient_type="rational"):
    ...
    # Generate random exponents - for addition, we can use the full max_degree
    # since we're not multiplying terms
    exponents = [random.randint(0, max_degree) for _ in range(len(variables))]
...
def generate_example(variables, max_terms, max_degree, num_addends=2, coefficient_type="rational", max_attempts=10):
    """Generate a single addition example with multiple polynomials."""
    for _ in range(max_attempts):
        try:
            # Create random polynomials
            polynomials = [
                create_polynomial(variables, max_terms, max_degree, coefficient_type)
                for _ in range(num_addends)
            ]
            
            # Compute their sum
            sum_poly = sum(polynomials)
            
            # Format all polynomials
            input_str = " [SEP] ".join(
                format_polynomial(poly, variables, coefficient_type)
                for poly in polynomials
            )
            output_str = format_polynomial(sum_poly, variables, coefficient_type)
...
def main():
    parser = argparse.ArgumentParser(description="Generate polynomial addition dataset")
    ...
    parser.add_argument("--num_addends", type=int, default=2, help="Number of polynomials to add")
    parser.add_argument("--coeff_type", choices=["rational", "real"], default="rational", help="Type of coefficients")
    parser.add_argument("--output_dir", type=str, default="data/addition", help="Output directory")
